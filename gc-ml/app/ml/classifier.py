from __future__ import annotations

from datetime import datetime
from typing import Any

from app.ml.scorer import score_gauge
from app.ml.recommender import recommend


def build_risk_payload(gauge: dict[str, Any], *, today: datetime | None = None) -> dict[str, Any]:
    if today is None:
        today = datetime.utcnow()

    # ── Step 1: Risk scoring (unchanged) ────────────────────────────────
    result   = score_gauge(gauge, today=today)
    features = result.features

    # ── Step 2: Recommendation (medium/high only, low returns no_change) ─
    rec = recommend(gauge, features, result.risk_level)

    # ── ml_features: risk features + predicted_overrun from recommender ──
    ml_features = {
        "days_until_due":    features.days_until_due,
        "overdue_count":     features.overdue_count,
        "completion_rate":   features.completion_rate,
        "avg_delay_days":    features.avg_delay_days,
        "frequency_months":  features.frequency_months,
        "is_overdue":        features.is_overdue,
        "next_due_date":     features.next_due_date,
        "max_delay_days":    features.max_delay_days,
        "history_size":      features.history_size,
        "predicted_overrun": rec.predicted_overrun_days,  # from Ridge model
    }

    # ── latest_prediction: risk fields + recommendation fields ───────────
    latest_prediction = {
        # risk (from scorer)
        "risk_score":  result.risk_score,
        "risk_level":  result.risk_level,
        # recommendation (from recommender)
        "action":                rec.action,
        "reason":                rec.reason,
        "signals":               rec.signals,
        "issue_type":            None,
        "recommended_frequency": rec.recommended_frequency,
        "recommended_due_date":  rec.recommended_due_date,
        "scored_at":             today,
    }

    return {"ml_features": ml_features, "latest_prediction": latest_prediction}